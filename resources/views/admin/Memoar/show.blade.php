...
@section('title', 'Detail Galeri: ' . $memoar->title)
            ...
            <div class="d-flex flex-column">
                <nav class="text-sm mb-1">
                    <div class="d-flex align-items-center gap-1 text-gray-500">
                        ...
                        <a href="{{ route('admin.memoar.index') }}" class="hover:text-gray-700">Memoar Wisata</a>
                        <span class="text-gray-400">›</span>
                        <span class="text-gray-900 font-semibold">Detail</span>
                    </div>
                </nav>
                <h5 class="text-lg font-semibold mt-3 mb-3">Detail Memoar Wisata</h5>
            </div>
        </div>
                        ...
                        <div class="memoar-header d-flex flex-column flex-md-row justify-content-between gap-3 mb-3">
                            <div class="d-flex align-items-center gap-3">
                                <h6 class="text-lg mb-0 text-neutral-600 dark:text-slate-200">
                                    by <span
                                        class="fw-semibold text-primary-light dark:text-primary-300">{{ $memoar->user->name }}</span>
                                </h6>
                            </div>
                            ...
                            <div
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <span>{{ $memoar->location }}</span>
                                </div>
                                <div class="d-flex align-items-center gap-2">
                                    <i class="ri-calendar-2-line"></i>
                                    <span>{{ $memoar->created_at->format('d M Y') }}</span>
                                </div>
                            </div>
                        </div>

                        {{-- Judul --}}
                        <h4 class="memoar-title dark:text-slate-100">{{ $memoar->title }}</h4>

                        {{-- Isi Memoar --}}
                        <div class="editor-content text-primary-light dark:text-slate-300">
                            {!! $memoar->content !!}
                        </div>
    ...
    <div class="modal fade" id="editStatusModal" tabindex="-1" aria-labelledby="editStatusModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content radius-12">
                ...
                <form action="{{ route('admin.memoar.updateStatus', $memoar->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="status" class="form-label">Status Memoar</label>
                            <select name="status" id="status" class="form-select">
                                <option value="aman" {{ $memoar->status === 'aman' ? 'selected' : '' }}>Aman</option>
                                <option value="ditahan" {{ $memoar->status === 'ditahan' ? 'selected' : '' }}>Ditahan
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="catatan_admin" id="catatan_admin" rows="4" class="form-control">{{ old('catatan_admin', $memoar->catatan_admin) }}</textarea>
                        </div>
                    </div>
