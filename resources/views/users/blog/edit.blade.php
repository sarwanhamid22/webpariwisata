...
@section('title', 'Edit Memoar | ' . $blog->title)
    ...
    <section class="relative md:px-3 mt-6 md:mt-0">
        <div class="container mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="p-6 rounded-md shadow-sm bg-white dark:bg-slate-900">
                ...
                <!-- Heading -->
                <h1 class="text-lg font-bold mb-6 text-gray-900 dark:text-white">
                    Edit Memoar Wisatamu
                </h1>

                <!-- Form -->
                <form action="{{ route('user.blog.update', $blog->slug) }}" method="POST" enctype="multipart/form-data"
                    id="blog-edit-form">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                            <div class="relative flex-1">
                                <input type="file" id="image" name="image" accept="image/*"
                                    ...
                                    onchange="document.getElementById('file-name').textContent = this.files[0]?.name || '{{ basename($blog->image) }}';">
                                <label for="image"
                    ...
                    <!-- Judul Memoar -->
                    <div class="mb-6">
                        <label for="title" class="block text-gray-900 dark:text-white font-medium mb-2">
                            Judul Memoar
                        </label>
                        <input type="text" id="title" name="title" value="{{ old('title', $blog->title) }}"
                            placeholder="Masukkan judul memoar" required
                            class="w-full p-3 bg-transparent dark:bg-slate-900 dark:text-slate-200 rounded-md border border-gray-200 dark:border-gray-700 focus:ring-0 focus:border-red-500 dark:focus:border-red-400">
                        @error('title')
                    ...
                    <!-- Nama Tempat Wisata -->
                    <div class="mb-6">
                        <label for="location" class="block text-gray-900 dark:text-white font-medium mb-2">
                            Nama Tempat Wisata
                        </label>
                        <input type="text" id="location" name="location" value="{{ old('location', $blog->location) }}"
                    ...
                    <!-- Konten Memoar (TinyMCE) -->
                    <div class="mb-6">
                        <label for="content" class="block text-gray-900 dark:text-white font-medium mb-2">
                            Isi Memoar
                        </label>

                        <textarea id="content" name="content" class="form-control radius-8" rows="12">
                {!! old('content', $blog->content) !!}
                    ...
                    <div class="mt-8 text-right">
                        <button type="submit"
                            class="inline-flex items-center justify-center gap-2 py-2 px-5 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-md transition-all duration-300">
                            <span>Perbarui Memoar</span>
                        </button>
                    </div>
...
@section('scripts')
    ...
    <script>
        ...
        var initialContent = @json(old('content', $blog->content));
        if (initialContent) quill.root.innerHTML = initialContent;

        document.getElementById('blog-edit-form').addEventListener('submit', function() {
