...
document.addEventListener('DOMContentLoaded', function () {
    const gallery = document.querySelector('.tiny-gallery-slider');
    if (gallery && typeof tns === 'function') {
      tns({
        container: gallery,
        items: 4,             // tampilkan 4 sekaligus
        slideBy: 1,
        autoplay: true,
        autoplayButtonOutput: false,
        controls: false,
  ...
  document.addEventListener('DOMContentLoaded', function () {
    // === Inisialisasi Tiny‑Slider untuk Review Slider ===
    const reviewSlider = document.querySelector('.tiny-review-slider');
    if (reviewSlider && typeof tns === 'function') {
      tns({
    ...
    // === Inisialisasi Rating Input ===
    const ratingValueEl = document.getElementById('rating-value');
    document.querySelectorAll('input[name="rating"]').forEach(radio => {
      radio.addEventListener('change', function() {
        // Update nilai teks
        if (ratingValueEl) {
          ratingValueEl.textContent = parseFloat(this.value).toFixed(1);
        }
        // Reset semua bintang
        document.querySelectorAll('#review-form .mdi-star').forEach(icon => {
          icon.classList.remove('text-amber-400');
          icon.classList.add('text-gray-300');
        });
        // Warnai bintang hingga yang dipilih
        for (let i = 1; i <= this.value; i++) {
          const starIcon = document.querySelector(`#star-${i}-icon`);
          if (starIcon) {
